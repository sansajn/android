...
#define LOG_TAG "vorbis_play_sample"

char const * sound_path = "/sdcard/test_files/test_clip.ogg";
...
JNIEXPORT void JNICALL Java_org_example_MainActivity_play_1sound(
	JNIEnv * env, jobject obj)
{
	if (!__as)
	{
		...
		__as->attach(std::shared_ptr<wave_data>{new vorbis_wave{sound_path}});
	}

	__as->play();
}

JNIEXPORT void JNICALL Java_org_example_MainActivity_update(
	JNIEnv * env, jobject obj)
{
	__as->update();
}

JNIEXPORT void JNICALL Java_org_example_MainActivity_free(
	JNIEnv * env, jobject obj)
{
	delete __as;
	openal_free();
