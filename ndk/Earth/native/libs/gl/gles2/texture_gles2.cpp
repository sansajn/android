...
{
	__android_log_print(ANDROID_LOG_DEBUG, "scene", "%s", "texture2d::read():begin");
	_w = width;
	_h = height;
	glPixelStorei(GL_UNPACK_ALIGNMENT, alignment_to(_w, pfmt, type));
	__android_log_print(ANDROID_LOG_DEBUG, "scene", "%s", "texture2d::read():glPixelStorei");
	...
	__android_log_print(ANDROID_LOG_DEBUG, "scene", "%s", "texture2d::read():glTexImage2D");
	// TODO: podpora mipmap ?
//	assert(glGetError() == GL_NO_ERROR && "opengl error");
	if (glGetError() != GL_NO_ERROR)
		__android_log_print(ANDROID_LOG_DEBUG, "scene", "%s", "texture2d::read():glGetError() failed");
	__android_log_print(ANDROID_LOG_DEBUG, "scene", "%s", "texture2d::read():end");
}
