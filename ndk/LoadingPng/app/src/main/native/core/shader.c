...
static void log_v_fixed_length(GLchar const * source, GLint const length)
{
	if (LOGGING_ON)
	{
		char log_buffer[length + 1];  // c99 ???
...
GLuint compile_shader(GLenum const type, GLchar const * source, GLint const length)
{
	...
	glShaderSource(shader_object_id, 1, (GLchar const **)&source, &length);
