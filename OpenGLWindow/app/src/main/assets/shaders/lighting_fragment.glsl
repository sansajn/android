...
struct PointLight
{
	...
	Attenuation atten;
	vec3 position;
	float range;  // do akej vzdialenosti svetlo uvazujem
};
...
//		float specularFactor = dot(directionToEye, reflectDirection);
		...
		if (specularFactor > 0.0)
			specularColor = vec4(base.color, 0) * specularIntensity * specularFactor;
	}
